...
for i in range(0, rainc24.shape[0]):
    #time string
    thour = 24 + 3 * i
    hour = thour % 24
    day = 16 + thour // 24
    timestr = f'2016-06-{day:02} {hour:02}:00:00 UTC'

    # save path & title
    dirpath = 'D:\\Repositories\\June11th-Jiangxi_case_figures\\fig_wrfout (scheme 1, 2 domains)\\c'
    if dim2 == '':
        save_path = os.path.join(dirpath,        dim1, )
    elif dim2 == 'nan, ':
        save_path = os.path.join(dirpath, dim2 + dim1, )
    ...
    plt.savefig(os.path.join(save_path, 'rain24h_'+timestr[:-10]+'.png'), dpi=300)
